...
Public Class manage_city1
    ...
    Protected Sub GVCity_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles GVCity.RowUpdating
        LblMsg.Text = ""

        Dim TxtStatee As TextBox = CType(GVCity.Rows(e.RowIndex).Cells(1).FindControl("TxtCity"), TextBox)
        Dim DDLCountryyy As DropDownList = CType(GVCity.Rows(e.RowIndex).Cells(1).FindControl("DDlState"), DropDownList)

        If CInt(DsCity.CheckOtherPrimary(TxtStatee.Text, DDLCountryyy.SelectedValue, GVCity.DataKeys(e.RowIndex).Value)) > 0 Then
            e.Cancel = True
            LblMsg.Text = "City [ " & TxtStatee.Text & " ] Already Exists , Please Try Again ! "
        Else
            LblMsg.Text = "City Updated Successfully"
        End If
    End Sub
    ...
    Protected Sub btnsearch_Click(sender As Object, e As EventArgs) Handles btnsearch.Click
        If TxtCity.Text = "" Then
            LblMsg.Text = "Enter Any City Name"
        Else

            GVCity.DataSource = DsCity.GetDataByname(TxtCity.Text)
            GVCity.DataBind()
